...
public class GeneratorBlock extends BlockWithEntity implements BlockEntityProvider {
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        if (!world.isClient) {
            ...
            NamedScreenHandlerFactory screenHandlerFactory = ((GeneratorBlockEntity) world.getBlockEntity(pos));
    ...
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        return checkType(type,ModBlockEntities.GENERATOR_BLOCK_ENTITY, (world1, pos, state1, blockEntity) -> blockEntity.tick(world1, pos, state1));
    }
